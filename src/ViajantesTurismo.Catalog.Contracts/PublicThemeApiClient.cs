...
public sealed class PublicThemeApiClient(HttpClient httpClient) : IPublicThemeApiClient
{
    ...
    {
        var theme = await httpClient.GetFromJsonAsync("/catalog/public-theme", Json.PublicThemeSettingsDto, ct).ConfigureAwait(false);
    ...
    public async Task<PublicThemeSettingsDto> SaveTheme(PublicThemeSettingsDto request, CancellationToken ct)
    {
        ...
        using var response = await httpClient.PutAsJsonAsync("/catalog/public-theme", request, Json.PublicThemeSettingsDto, ct).ConfigureAwait(false);
