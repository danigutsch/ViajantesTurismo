...
public sealed class GeneratorImplementationTests
{
    ...
    public void Catalog_models_with_different_entries_are_not_equal()
    {
        ...
        (matchesDifferentObject).ShouldBeFalse();
    }
    ...
    public void Build_discovers_static_error_providers_and_extracts_metadata()
    {
        ...
        var invalidEntry = (entries).ShouldHaveSingleItem(static entry => entry.MemberName == "MissingName");
        (invalidEntry.Identifier).ShouldBe("demo-usererrors-missingname");
        (invalidEntry.Status).ShouldBe("Invalid");
        (invalidEntry.Code).ShouldBe("invalid");
        (invalidEntry.DetailTemplate).ShouldBe("Name is required.");

        var errorEntry = (entries).ShouldHaveSingleItem(static entry => entry.MemberName == "SaveFailed");
        (errorEntry.Status).ShouldBe("Error");
        (errorEntry.Code).ShouldBe("error");
    }
    ...
    public void Emit_writes_auto_generated_header_and_provider_class()
    {
        ...
        // Assert
        (generated).ShouldContain("// <auto-generated />", StringComparison.Ordinal);
        (generated).ShouldContain("GeneratedDemoResultErrorCatalogProvider", StringComparison.Ordinal);
        (generated).ShouldContain("public global::System.Collections.Generic.IReadOnlyList<ResultErrorCatalogEntry> Entries => ResultErrorCatalogGenerated.generatedEntries;", StringComparison.Ordinal);
    }
