...
public sealed class ServiceDefaultsPrivacyTelemetryTests
{
    ...
    public void Trace_export_removes_sensitive_attributes_and_status_description_but_keeps_bounded_error_type()
    {
        ...
        using (var activity = activitySource.StartActivity("privacy.failure", ActivityKind.Internal))
        {
            ...
            activity.SetTag("db.query.text", $"select * from customers where email = '{email}'");
            activity.SetTag("db.statement", $"select * from customers where email = '{email}'");
            activity.SetTag("db.parameters", email);
            activity.SetTag("db.query.parameter", email);
            activity.SetTag("db.query.parameter.customer_email", email);
            activity.SetTag("db.npgsql.data_source", "Host=localhost;Username=private;Password=secret");
            activity.SetTag("db.npgsql.connection_id", customerId.ToString());
            activity.SetTag("media.object_key", objectKey);
            activity.SetTag("aws.s3.bucket", "private-media");
            activity.SetTag("aws.s3.key", objectKey);
            activity.SetTag("aws.sqs.queue.url", $"https://sqs.example.test/{customerId}");
            activity.SetTag("aws.sns.topic.arn", $"arn:aws:sns:eu-west-1:123456789012:{bookingId}");
            activity.SetTag("messaging.destination.name", customerId.ToString());
            activity.SetTag("messaging.message.id", bookingId.ToString());
        ...
        exported.GetTagItem("db.query.text").ShouldBeNull();
        exported.GetTagItem("db.statement").ShouldBeNull();
        exported.GetTagItem("db.parameters").ShouldBeNull();
        exported.GetTagItem("db.query.parameter").ShouldBeNull();
        exported.GetTagItem("db.query.parameter.customer_email").ShouldBeNull();
        exported.GetTagItem("db.npgsql.data_source").ShouldBeNull();
        exported.GetTagItem("db.npgsql.connection_id").ShouldBeNull();
        exported.GetTagItem("media.object_key").ShouldBeNull();
        exported.GetTagItem("aws.s3.bucket").ShouldBeNull();
        exported.GetTagItem("aws.s3.key").ShouldBeNull();
        exported.GetTagItem("aws.sqs.queue.url").ShouldBeNull();
        exported.GetTagItem("aws.sns.topic.arn").ShouldBeNull();
        exported.GetTagItem("messaging.destination.name").ShouldBeNull();
        exported.GetTagItem("messaging.message.id").ShouldBeNull();
