...
repository-owned .NET tooling from resolved `packages.lock.json` files. License values that lock files
cannot prove are marked `NOASSERTION` for release review.
