...
4. Run `SharedKernel.Versioning.Tool validate-package-metadata` for required package metadata.
5. Run `SharedKernel.Versioning.Tool api-compatibility` to write the package compatibility report.
6. Run `SharedKernel.Versioning.Tool prepare-release` to generate `release-notes.md`, `CHANGELOG.md`,
   `release-manifest.json`, `sbom.spdx.json`, `third-party-attributions.json`, and
   `third-party-notices.md`.
7. Run `aspire publish` with the computed version values in environment/MSBuild inputs so application
   container artifacts use the same release version.
8. Upload package, Aspire, and release-prep artifacts for review.
...
The release-prep manifest records package file names, SHA-256 hashes, sizes, source SHA, version
metadata, SBOM path, and third-party attribution paths. The SBOM and attribution files are generated by
repository-owned .NET tooling from resolved lock files and package outputs. License values that lock
files cannot prove are marked `NOASSERTION` for release review.
